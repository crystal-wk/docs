# Source: https://github.com/bytedance/deer-flow/blob/main/backend/docs/IM_CHANNEL_CONNECTIONS.md
...
[History](https://github.com/bytedance/deer-flow/commits/main/backend/docs/IM_CHANNEL_CONNECTIONS.md)

Open commit details

History

465 lines (353 loc) · 29.6 KB
...
[Raw](https://github.com/bytedance/deer-flow/raw/refs/heads/main/backend/docs/IM_CHANNEL_CONNECTIONS.md)
...
This document covers both **architecture** (how the bind / dispatch / file pipeline fits together) and **configuration / operations** (the existing `config.yaml` knobs and security notes). For the high-level orientation, see [AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md) → "IM Channels System".
...
For the special GitHub case (`fire_and_forget=True` channel policy), the manager calls `runs.create()` and returns once the run is `pending` — no outbound reply, because GitHub agents post via the `gh` CLI from inside their sandbox. See [GITHUB\_AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/docs/GITHUB_AGENTS.md) for the full GitHub flow.
...
- [AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md) → "IM Channels System" — the index view in `backend/AGENTS.md` (configuration knobs, message flow, component list)
- [GITHUB\_AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/docs/GITHUB_AGENTS.md) — webhook-driven GitHub channel, agent bindings, fan-out, token lifecycle
...
- **Membership is scoped to live events.** The relay _stores_ 44100/44101 events, so without a `since` every connection replayed the whole membership history as if it had just happened — re-running channel discovery once per stored add (you would see several `channel discovery complete` lines for one connect, and channels logged as `<unnamed>`), re-subscribing channels you have since been removed from, and briefly unsubscribing channels you are still in. The subscription is therefore anchored at the moment the socket opened, minus 60s of slack so a membership change made _during_ the connect/auth handshake — or a small relay clock skew — is still picked up.
...
**Trusted (not verified):** the _authorship_ of kind-39000 channel metadata. Buzz publishes channel discovery events from the relay's own keypair, but nothing already configured identifies that key (`relay_url` is a network address, not a signing key), so DeerFlow only proves such an event was signed by _some_ member. Because channel discovery and subscription are now driven by exactly these events, a forged kind-39000 has two effects, not one:
...
- Stored per-connection credentials use the `channel_credentials` encryption path. If stored credential material cannot be decrypted, DeerFlow treats it as unavailable instead of using corrupt secrets.
- The local plaintext runtime credential fallback is documented above; prefer deployment-managed environment/config secrets for non-local deployments until a dedicated secret backend is configured.
