# Source: https://github.com/bytedance/deer-flow/blob/main/backend/docs/MCP_SERVER.md
...
[History](https://github.com/bytedance/deer-flow/commits/main/backend/docs/MCP_SERVER.md)

Open commit details

History

333 lines (274 loc) · 12.6 KB
...
[Raw](https://github.com/bytedance/deer-flow/raw/refs/heads/main/backend/docs/MCP_SERVER.md)
...
Two independent timeouts bound stdio MCP servers. `session_init_timeout` covers server bring-up — tool discovery (subprocess spawn + `initialize` + `tools/list`) and persistent-session initialization — and defaults to 60s so a hung server (e.g. `npx` blocked on a package download, or a server that never answers `initialize`) cannot block agent construction indefinitely. Set it to `null` to disable:
...
`tool_call_timeout` limits each individual tool call in seconds and applies only to `stdio` servers; `http` and `sse` servers use transport-level timeouts, and DeerFlow logs a warning if `tool_call_timeout` is configured for those transports.
