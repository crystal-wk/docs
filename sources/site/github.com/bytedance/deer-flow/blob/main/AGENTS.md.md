# Source: https://github.com/bytedance/deer-flow/blob/main/AGENTS.md
...
[History](https://github.com/bytedance/deer-flow/commits/main/AGENTS.md)

Open commit details

History

162 lines (131 loc) · 9.94 KB
...
[Raw](https://github.com/bytedance/deer-flow/raw/refs/heads/main/AGENTS.md)
...
- **[backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md)** — backend depth: harness/app split, agent & middleware chain, sandbox, MCP, skills, memory, IM channels, persistence/migrations, config system, test layout.
- **[frontend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/frontend/AGENTS.md)** — frontend depth: Next.js App Router layout, thread/streaming data flow, code style, commands.
...
Nginx is the single public entry: it serves the frontend and proxies `/api/langgraph/*` to the Gateway's LangGraph runtime, rewriting it to Gateway's native `/api/*` routes; all other `/api/*` go straight to the Gateway REST routers. See [backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md) for the runtime and router detail. It compresses HTML and configured textual assets, while deliberately leaving SSE, fonts, images, audio, and video uncompressed at the proxy layer.
...
Third-party extensions are loaded from a top-level `plugins:` list in `config.yaml` (operator-controlled on purpose — that list causes code to be imported, so it is deliberately kept out of the API-writable `extensions_config.json`). See the Extension System section in [backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md).

Runtime config lives at the **repo root**: copy `config.example.yaml` → `config.yaml` (main app config) and `extensions_config.example.json` → `extensions_config.json` (MCP servers + skills). Both real files are gitignored and may be edited at runtime via the Gateway API. Config schema and resolution order are documented in [backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md).

Skill quality review note:

- `skills/public/skill-reviewer/` is the built-in read-only skill quality reviewer. It uses the harness-layer `review_skill_package` tool and contracts in `contracts/skill_review/`. Model-visible review data is compact and tag-neutralized; full raw payloads stay in tool artifacts. See [backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md) for the non-activation, SkillScan, and `skill-creator` ownership boundaries.
...
- Backend work → **[backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md)**
- Frontend work → **[frontend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/frontend/AGENTS.md)**
- Setup & install → **[Install.md](https://github.com/bytedance/deer-flow/blob/main/Install.md)**, **[CONTRIBUTING.md](https://github.com/bytedance/deer-flow/blob/main/CONTRIBUTING.md)**
- Project overview & usage → **[README.md](https://github.com/bytedance/deer-flow/blob/main/README.md)** (translations: `README_zh.md`, `README_ja.md`, `README_fr.md`, `README_ru.md`)
- Security policy → **[SECURITY.md](https://github.com/bytedance/deer-flow/blob/main/SECURITY.md)**
- Changes → **[CHANGELOG.md](https://github.com/bytedance/deer-flow/blob/main/CHANGELOG.md)**
- Cutting a release → **[RELEASING.md](https://github.com/bytedance/deer-flow/blob/main/RELEASING.md)**
...
- **Test-driven development** — features and bug fixes ship with tests. Backend tests live in `backend/tests/` (TDD is mandatory there; see [backend/AGENTS.md](https://github.com/bytedance/deer-flow/blob/main/backend/AGENTS.md)); frontend tests live in `frontend/tests/`.
